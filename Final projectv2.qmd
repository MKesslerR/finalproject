...
Walking is the most natural form of movement around our environment and occurs in multiple diverse contexts (Morris & Hardman, 1997). Walking can occur both as an integrative practice and a dispersed practice (Harries & Rettie, 2016). Walking as an integrative practice occurs when the purpose of walking is to actually experience the walk. In contrast, dispersed walking is walking that occurs as part of a different integrative practice, such as shopping or traveling to work; walking is not the purpose of the practice, but a means to achieve it (Westgard et al. 2021). 
...
How do recreational walks differ from functional dog walks?
Are dog walks influenced by weather (precipitation)? 
How do the recreational dog walking patterns differ throughout the day?
What are the movement behavior patterns during dog walks?
...
ggplot(Maxdist_summary, mapping = aes(Type, max_distance))+
  ...
  theme_classic()
```
One of the dog owners always started the dog walks from home, whereas the other dog owner occasionally drove to another location for the walks (Fig. 5). 
```{r fig.cap= "Figure 5: Visualization of all home trajectories for both users."}
...
```
According to the Frechet distances, functional walk trajectories were more dissimilar than recreational trajectories (Fig. 6). However, we detected high variability in both types of trajectories. 
```{r results = 'hide'}
# Frechet Distance
...
```{r fig.cap= "Figure 6: Boxplot of frechet distance for the two types of walks"}
...
```{r fig.cap= "Figure 7: Frechet distances calcualted among three functional and three recreational trajectories for two dog owners."}
...
Distance covered during dog walks varied throughout the day (Df = 2, F-value = 8.44, p < 0.001). Morning dog walks were on average longer than in the afternoon, and evening walks were the shortest on average (Fig. 9).  Neither the length (t = 0.63, df = 53.73, p = 0.53) nor the duration of the walks (t = -0.19 secs, df = 48, p = 0.84) were affected by the day of the week. During weekdays, the length of dog walks were on average 3565.23 m, while on weekends the average was 3236.9 m. 

```{r fig.cap= "Figure 9: Boxplot displaying distance covered for each time of the day category."}
...
The spatial movement behavior while dog walking ranged from axial movement to full-loop circuits. The most common spatial movement behavior was full-loops, followed by semi-loops, and the least common was axial trajectories (Fig. 10). In full-loop trajectories, walks occur without any repetition of routes. In semi-loop trajectories, the user moves partially over the axial route to a certain point, then starts a circuitous path to return home. In axial trajectories, the user takes a direct route from their starting point, stops at a certain point, then subsequently returns home using the same route.

```{r fig.cap= "Figure 10: Pie chart displaying the trajectory shapes in percentage of all walks."}
...
```{r fig.cap= "Figure 11: Vizualization of the three trajectory shapes."}
loop_plot <- posmo  |> 
