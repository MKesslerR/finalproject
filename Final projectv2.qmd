...
mean(Maxdist_summary$max_distance[34:103]) # average distance of recreational walks 3484.34 m
...
ggplot(dist_summary, mapping = aes(Type, distance))+
  ...
  theme_light()
```
Figure 4 shows the average maximum distance distance covered from home, which was 624.39 m for functional walks and 3484.34 m for recreational walks (please note that two outliers were removed from the plot).
...
Most of the trajectories while dog walking were loops and semi-loops and, the less common were axial trajectories (Fig. 8). 
...
As we had predicted the longest walk usually occurred in the morning. Morning walks are a way to boost energy levels and improve your mood. They provide exercise opportunities for both dogs and dog owners after, and in the absence of time constraints, it seems the most adequate time for long and pleasant walks. At night, the walks are usually shorter and usually functional in nature.  
...
```{r}
test <- posmo |> group_by(Type, trajectoryID) |> 
  count(trajectoryID)
