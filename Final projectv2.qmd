...
How do the recreational dog walking patterns differ through the day?
...
### **Data set**
We tracked the daily daily movement behavior of two users with the tracking app Posmo over the course of about two months (April & May 2023). The app tracked the user location every 15 seconds and detected transport mode. We used only tracking data during dog walks. The final data set contained 25´455 data points of 103 trajectories in total. The following attributes were part of the movement data set of the two tracker users:
...
We then extracted the precipitation data from two weather stations closer to the two hometowns of the two users (Hinwil and Schaffhausen). We kept the three attributes that were relevant to us:
...
We filtered tracking data in walking mode and separated manually those of dog walking (users input). The app created duplicate time stamps with different sets of coordinates that had to be cleaned out. Several pre-processing steps had to be done before we could join the two users' data sets. First, we stored the X and Y coordinates into two separate columns. Next, the corresponding precipitation data was joined to the individual movement data sets. This process is defined as a semantic enrichment process, which complements the existing data with additional data (Parent et al. 2013). A new column was added for rain/no rain. Trajectories that had precipitation more than 50% of the walk were categorized as having rain and assigned "no rain" for values below this threshold. For each trajectory we calculated the longest distance home. New columns were added for the weekday (weekend/weekday) and time of the day (morning/afternoon/evening). Next, we segmented the trajectories and eliminated static points with the method used by Laube and Purves (2011). A threshold of the average step mean was applied to eliminate static positions (Fig. 1). However, after filtering static points, several trajectories were left out and therefore subsequent calculations were done with the original data set. Further parameters were calculated and stored in new columns such as step mean, time lag, step length, speed, duration and distance.   
...
### **Analysis**
We evaluated differences between functional and recreational dog walks in speed, distance and maximum distance from the house with t-tests. Randomly, we chose three functional and three recreational trajectories for each of the two subjects. Then we evaluated similarity by calculating Frechet distance among the trajectories within trajectory type and within subject. 
...
#                  1.202219                   1.214331 
```
Functional dog walks had an average speed of 1.20 m/s while the average speed of recreational dog walks was 1.21 m/s (Fig. 2), but they weren't significantly different (t = -0.38, df = 100, p = 0.70).
...
ggplot(speed_summary, mapping = aes(Type, speed))+
  ...
  theme_classic() 
```
The average distance of functional walks was 1272.11 m, compared to 4389.43 m for recreational walks (Fig. 3).
...
```
According to the Frechet distances, functional walk trajectories were very dissimilar, and the same was observed for recreational trajectories (Fig. 5).
```{r results = 'hide'}
# Frechet Distance
...
#Similarity measures: Calculate Frechet distance for 3 randomly selected functional and 4 recreational walks within the same walker. 
run_full <- FALSE
if(run_full){
...
The spatial movement behavior while dog walking ranged from axial movement to full-loop circuits. The more common spatial movement behavior was full-loops followed by semi-loops and the less common was axial trajectories (Fig. 9). In the full-loop trajectories the walk occurs without any repetition of the routes. In the semi-loop trajectories, the user move partially over the axial route to a certain point and then start a circuitous path to return home. In the axial routes, the user take a direct route from their starting point to a certain point and subsequently, they return home taking the same route.
...
Our research aimed to characterize two types of dog walking practices: functional and recreational. Additionally, we sought to evaluate temporal differences and the effect of the weather on the dog walking behavior. According to Westgarth et al. (2021), functional walks are usually performed to meet the perceived practical needs of the dog to undertake some exercise and excretion. Recreational walks are used to de-stress and relax the owner, particularly after work or at weekends. During our classification of the two types of walking practices, we initially set the threshold of functional walks at 15 minutes, but only a few trajectories meet this condition. Then we set the threshold at 20 minutes but still none of the walks of one of the participants met this condition. We finally set the threshold of 30 minutes. This threshold is backed up by the recommendation of the US Department of Agriculture which advises that dogs should have at least 30 min of exercise per day (Pickup et al. 2017). 

Our results agreed with our predictions, functional walks were usually taken close to home, in the neighborhood, because these were the most convenient places when short on time. We had also predicted that the pace of functional walks would be faster than recreational walks. However, we found no differences. Probably, for both dog owners it is the dog who sets the pace of the walk. Or that under time constraints, both owners chose short routes, and hence the time constraint didn’t affect the pace. 

We predicted that the trajectories followed during functional walks were more similar to one another than recreational walks. The rationale behind it was that there were fewer options when walking the dog under a time constraint or that when time was limited, the dog owners have a few favorite-practical routes. Yet, the Frechet distances calculations suggest, that the differences among trajectories of functional walks were as large as those among recreational trajectories (Figure 6). This indicates that dog owners don’t have favorite routes for functional or recreational dog walks. However, these results should be interpreted cautiously since the analyses only included three trajectories of each type of walk for each dog owner. It is likely that if the analysis included all the trajectories, other patterns may have emerged. 

The dog-owner relationship it is thought to be the most important determinant of dog walking behavior (Westgarth et al. 2014). During this trial, most of the walks were recreational. This suggests that for both dog owners, the dog walking activity is an integrative practice, enjoyable for both dogs and dog owners.  

**The effect of weather on the dog-walking behavior**

We had initially predicted that in poor weather walks would often be more functional in nature, shorter in duration, and at a faster pace. But we had very few walks in poor weather to find meaningful patterns. Our results indicate that under rainy conditions the dog walks have a higher pace but weren’t shorter than under dry conditions. Since both pet owners have flexible schedules for the walks most of the week, it is possible that the walks were timed in between rain episodes or when the rain was light and therefore the walks weren’t shorter but only at a faster pace to avoid being caught in the rain.

**Temporal differences in dog walking behavior**

We expected that weekend walks would be longer than those on weekdays due to time constraints. We also predicted that morning dog walks will cover longer distances than afternoon and evening walks. We did not find differences in the walks' length between weekdays and weekends probably because of the flexibility in the schedule of both dog owners. The expected pattern would be more likely to be found with dog owners with a tight working schedule.

As we had predicted, the longest walk usually occurred in the morning. Morning walks are a way to boost energy levels and improve mood. They provide exercise opportunities for both, dogs and dog owners. In the absence of time constraints, it seems the most adequate time for long and pleasant walks is the morning. In the evening, the walks were usually shorter and functional in nature.  

**Use of tracking data for characterizing dog walking behavior**

Using tracking data to characterize a specific type of walking, namely dog walking, was challenging because it was necessary to have the input of the users to separate dog walking from other kinds of walking. It would be even more complicated with data sets from a more extended period when it is harder to recollect the daily activities. The double time stamps made the pre-processing difficult and time-consuming since it took time to decide which coordinates were correct. Tracking with a mobile device has limitations regarding the accuracy of positioning under dense forest conditions, which can affect speed estimations. Despite of this limitations we were able to characterize the two types of dog-walking behavior. 

To summarize, the weather and time affect the intention of dog walking. Dog walks under rainy conditions were not shorter but faster, and dog owners seemed to mitigate bad weather conditions by adjusting the time of their walks. The length of the dog walks changed throughout the day, with longer walks in the morning and shorter walks in the evening. Mobile tracking data were adequate for the purpose of this study, but it would be necessary to be complemented with annotations of the users. 
