...
Walking is the most natural form of movement around our environment and occurs in multiple diverse contexts (Morris & Hardman, 1997). Walking can occur both as an integrative practice and a dispersed practice (Harries and Rettie, 2016). Walking as an integrative practice occurs, where the purpose of the walking is actually to experience the walk. In contrast, dispersed walking is walking that occurs as part of a different integrative practice, such as shopping or traveling to work; walking is not the purpose of the practice but a means to achieve it (Westgard et al. 2021). 
...
With the prevalence of personal mobile devices, a significant amount of positioning data capturing people's movements can be gathered. Nevertheless, there remains a substantial disparity between the raw trajectory data, which represents the basic geographic coordinates detected by mobile devices, and the comprehension of human behavior. There is a clear requirement to analyze these raw trajectories and extract behavioral patterns in order to interpret them and gain novel insights into mobility behavior (Renso et al. 2013). We attempted to use tracking data from personal devices of two dog owners to characterize the two types of dog walking behaviors. Our research questions are the following:
...
Further parameters were calculated and stored in new columns after static points were removed, such as step mean, time lag, step lenght, speed, duration and distance. 
...
posmo_filter <- posmo |>
    filter(!static)
```
Figure 1 below shows a trajectory with- (left) and without static points (right). A threshold of the average step mean of 50 meters was applied to filter out the static points that are below the defined threshold.
```{r fig.cap= "Figure 1: Vizualization of a trajectory with and without static points."}
...
#                  1.202219                   1.214331 
```
Functional dog walks took place at an average speed of 1.20 m/s while the average speed of recreational dog walks was 1.21 m/s (see Figure 2). The t-test showed no significant differences between them (t = -0.38, df = 100, p-value = 0.70).
...
ggplot(speed_summary, mapping = aes(Type, speed))+
  ...
  theme_light() 
```
Figure 3 shows the average distance of functional walks of 1272.11 m, compared to 4389.43 m for recreational walks.
...
ggplot(dist_summary, mapping = aes(Type, distance))+
  ...
  theme_light()
```
Figure 4 shows the average maximum distance distance covered from home, which was 624.39 m for functional walks and 1833.64 m for recreational walks (two outliers were not included in the calculation).
...
```{r fig.cap= "Figure 5: According to the Frechet distances, functional walk trajectories were very dissimilar, and the same was observed for recreational trajectories."}
...
Rainy weather did not affect the distance covered during dog walks (t = -0.13733, df = 8, p-value = 0.89). In rainy weather, the average length of dog walks was 3577.27 m whereas under no rain the average length was 3453.63 m. However, the weather conditions had an effect on the speed of the dog walking behavior. Under rainy conditions, the speed of dog walks was faster than when there was no rain (t = -4.3871, df = 27.165, p-value < 0.0001) (see Figure 6). The average speed was 1.20 m/s for walks without precipitation, compared to an average speed of 1.30 m/s for walks with precipitation. There were only 8 walks (7.8%) with precipitation, compared to 95 walks (92.2 %) without precipitation.
```{r fig.cap= "Figure 6: Boxplot displaying speed for walks with- and without precipitation."}
...
**Temporal Differences in Dog Walking Behavior**
Distance covered during dog walks varied throughout the day (Df = 2, Sum Sq = 78656052, F value = 8.444, p < 0.001). Morning dog walks were on average longer than in the afternoon, and the evening walks were the shortest on average (Fig. 7).  Neither the length (t = 0.63, df = 53.73, p-value = 0.53) nor the duration of the walks was (t = -0.19 secs, df = 48, p-value = 0.84) affected by the day of the week. During weekdays the length of dog walks were on average 3565.23 m, while on weekends the average was 3236.9 m. 
```{r fig.cap= "Figure 7: Boxplot displaying distance covered for each time of the day category."}
...
The spatial movement behavior while dog walking ranged from axial movement to full-loop circuits. The more common spatial movement behavior was loops followed by semi-loops and the less common was axial trajectories (Fig. 8). In the full-loop trajectories the walk occurs without any repetition of the routes. In the semi-loop trajectories, the dog and dog owner move partially over the axial route to a certain point and then start a circuitous path to return home. In the axial routes, the dog owner and dog take a direct route from their starting point to a certain point and subsequently, they return home taking the same route.
```{r fig.cap= "Figure 8: Pie chart displaying the trajectory shapes in percentage of all walks."}
...
```{r fig.cap= "Figure 9: Visualization of all home trajectories for both users."}
...
**Comparison of functional and recreational dog-walks**
Our research aimed to characterize two types of dog walking practices: functional and recreational. Additionally, we sought to evaluate temporal differences and the effect of the weather on the dog walking behavior. According to Westgarth et al. (2021), functional walks are usually performed for a simple, functional purpose, usually to meet the perceived practical needs of the dog to undertake some exercise and excretion. Recreational walks were used to de-stress and relax the owner, particularly after work or at weekends. Functional dog walks are usually short in distance and duration and occur close to home and in bad weather (Westgarth et al. 2021). During our classification of the two types of walking practices, we initially set the threshold for functional walks as shorter than 15 minutes, but only a few trajectories meet this condition. Then we set the threshold as 20 minutes but still, none of the walks of one of the participants met this condition. We finally set it as 30 minutes, backed by the US Department of Agriculture's recommendation that dogs should have at least 30 min of exercise per day (Pickup et al. 2017). Our results agreed with our predictions; functional walks usually occur close to home, in the neighborhood, because these were the most convenient places when short on time. We had also predicted that the pace of functional walks would be faster than recreational walks. However, we found no differences. Probably, for both dog owners is the dog who sets the pace of the walk. Moreover, when there were time constraints, both owners could just choose short routes, so the time constraint didn’t affect the pace. 
We predicted that the trajectories followed during functional walks were more similar to one another than recreational walks. The rationale behind it was that there were fewer options when walking the dog under a time constraint or that when time was limited the dog owners have a few favorite-practical routes. Yet, the Frechet distances calculations suggest that the differences among trajectories of functional walks were as large as those among recreational trajectories (Fig. x). This suggests that dog owners don’t have favorite routes for functional or recreational dog walks. Nevertheless, these results should be interpreted cautiously since only four randomly chosen trajectories of each type for each dog owner were included in the analysis. It is likely that if the analysis included all the trajectories, other patterns may have emerged. Additionally, both dog owners live in rural areas with many options for dog walking and dog owners usually look for various routes to give their dogs opportunities of experiencing new sights and smells (Aspling 2015, Fletcher and Platt 2016).
The dog-owner relationship appears to be the most important determinant of dog walking behavior. The dog-owner relationship includes factors such as attachment, frequency of interactions, feelings of support and motivation provided by the dog for walking, and feelings of obligation towards walking the dog (Westgarth et al. 2014). During this trial, most of the walks were recreational which suggests that for both dog owners, the dog walking activity is an integrative practice enjoyable for both dogs and dog owners. 
**The effect of weather on the dog-walking behavior**
We had initially predicted that in poor weather (rainy conditions) walks would often be more functional in nature, shorter in duration, and at a faster pace. But we have very few walks in poor weather to find meaningful patterns. Our results indicate that under rainy conditions the dog walks have a higher pace but weren’t shorter than under dry conditions. Since both pet owners have flexible schedules most of the week, it is possible that they adjusted their walk time in between rain episodes or when the rain was light and therefore the walks were not shorter but only at a faster pace to avoid being caught in the rain.

**Temporal differences in dog walking behavior**
We expected that weekend walks would be longer than those on weekdays due to time constraints. We also predicted that morning dog walks will cover longer distances than afternoon and evening walks. We did not find differences in the length between weekdays and weekends since both dog owners have the flexibility to arrange the timing of the walks most weekdays. This pattern would be more likely to be found with dog owners with a tight working schedule.
As we had predicted, the longest walk usually occurred in the morning. Morning walks are a way to boost energy levels and improve mood. They provide exercise opportunities for both dogs and dog owners, and in the absence of time constraints, mornings seem the adequate time for long and pleasant walks. At night, the walks are usually shorter and usually functional in nature.  
...
```{r results = 'hide'}
# Plots that are not going to be included in the report (could get deleted)
#Visualization of parameters
...
ggplot(duration_wday_summary, mapping = aes(wday, duration))+
  geom_boxplot() +
  labs(x = "Day", y = "Duration [mins]")+
  theme_light()

#ggplot(starttime_wday_summary, mapping = aes(wday, start))+
#  geom_point() +
