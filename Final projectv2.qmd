...
T1 <- posmo_MK %>%
 ...
 max (st_distance(T1, house_MK, by_element = FALSE))
 
 T2 <- posmo_MK %>%
  filter(trajectoryID == "T2")

 max (st_distance(T2, house_MK, by_element = FALSE))

dist_home <- posmo_MK |>
  group_by(trajectoryID) |>
  summarise(max_distance = max(st_distance(house_MK))) #Put point in st_distance()
```
...
>>>>>>> a1f114bfe5d6f06604f1908aad9b5c9c7c898b2e
