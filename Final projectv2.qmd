...
plot_grid(K1a, K1b, 
         labels = c("Raw", "Static points removed"),
          ...
          ncol = 2, nrow = 1)
```
Plot 1: Vizualization of a trajectory with and without static points
```{r results = 'hide'}
...
Functional dog walks took place at a speed of 1.20 m/s while the average for recreational dog walks was 1.21 m/s., and there weren’t significant differences between them (t = -0.38, df = 100, p-value = 0.70) 
```{r}
...
ggplot(speed_summary, mapping = aes(Type, speed))+
  ...
  theme_light() #use this one for the report. Fig 2
```
Figure 2: Boxplot displaying speed for the two types of walks

Functional walks covered shorter distances (Fig. 3) and took place at closer distances from the house than recreational dog walks (Fig. 4).
```{r}
ggplot(dist_summary, mapping = aes(Type, distance))+
  geom_boxplot() +
  labs(x = "Type of dog walking", y = "Distance [m]")+
  theme_light()#Figure 3

ggplot(Maxdist_summary, mapping = aes(Type, max_distance))+
  geom_boxplot() +
  labs(x = "Type of dog walking", y = "Max distance home [m]")+
  theme_light()+
  scale_y_continuous(limits = c(0, 1000))#Fig 4
...
plot_ly(shape_summary, labels = ~Shape, values = ~percent, type = 'pie') %>%
  layout(title = "Trajectory Shapes",          
         ...
         yaxis = list(showgrid = FALSE))#Fig. 8
```
...
plot_grid(plotMN, plotMK, align = "hv",
         ...
         ncol = 2, nrow = 1, vjust = 3) Figure 9
